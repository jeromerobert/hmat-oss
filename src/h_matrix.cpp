...
template <typename T>
void HMatrix<T>::profile(HMatProfile & result)
{
  size_t r = rows()->size();
  size_t c = cols()->size();
  if(r == 0 || c == 0) {
...
HMatrix<T>::recursiveGemm(char transA, char transB, T alpha, const HMatrix<T>* a, const HMatrix<T>*b, bool dig_a, bool dig_b, bool dig_c) {
    ...
    const int row_a = (!dig_a ? 1 : (transA=='N' ? a->nrChildRow() : a->nrChildCol()));
    const int col_a = (!dig_a ? 1 : (transA=='N' ? a->nrChildCol() : a->nrChildRow()));
    const int row_b = (!dig_b ? 1 : (transB=='N' ? b->nrChildRow() : b->nrChildCol()));
    const int col_b = (!dig_b ? 1 : (transB=='N' ? b->nrChildCol() : b->nrChildRow()));
    const int row_c = (!dig_c ? 1 : this->nrChildRow());
    const int col_c = (!dig_c ? 1 : this->nrChildCol());
