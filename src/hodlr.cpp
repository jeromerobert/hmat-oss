...
void factorizeSym(HMatrix<T> * a, HODLRNode<T> * node) {
  ...
  ata.lltDecomposition();
  // Compute I + L^t.K.L in node->x11
  node->x11.trmm(Side::RIGHT, Uplo::LOWER, 'N', Diag::NONUNIT, 1, &ata);
  node->x11.trmm(Side::LEFT, Uplo::LOWER, 'T', Diag::NONUNIT, 1, &ata);
  node->x11.addIdentity(1);
  // Compute M11 in node->x11
  node->x11.lltDecomposition();
  node->x11.addIdentity(-1);
  // Compute X11
  node->x11.trsm(Side::RIGHT, Uplo::LOWER, 'N', Diag::NONUNIT, 1, &ata);
  node->x11.trsm(Side::LEFT, Uplo::LOWER, 'T', Diag::NONUNIT, 1, &ata);
  ...
  ScalarArray<T> ixutu(r, r);
  ixutu.addIdentity(1);
  ixutu.gemm('N', 'N', 1, &node->x11, &ata, 1);
