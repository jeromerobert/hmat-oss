...
int set_diagonal_children(hmat_matrix_t* holder, hmat_matrix_t** holder_children) {
  ...
  unsigned nrDofChild = newChild->rowsTree()->data.coordinates()->numberOfDof();
  while (!stack.empty()) {
    try {
      ...
      const unsigned nrDof = m->rowsTree()->data.coordinates()->numberOfDof();
      if (nrDof == nrDofChild) {
        ...
        for (int i = 0; i < father->nrChildRow(); ++i) {
          if (father->get(i, i) == m) {
            ...
            if (!stack.empty()) {
              ...
              nrDofChild = newChild->rowsTree()->data.coordinates()->numberOfDof();
            }
            break;
          }
        }
